document.addEventListener('DOMContentLoaded', function () {
  fetch(
    'http://localhost/senalink5/senalink5/senalink/controllers/UsuarioController.php?action=detalleUsuario',
    {
      credentials: 'include',
    }
  )
    ...
    .then((text) => {
      ...
      if (userData.rol === 'empresa') {
        ...
        document.getElementById('representante_legal').textContent =
          userData.representante_legal || '';
        document.getElementById('tipo_empresa').textContent =
          userData.tipo_empresa || '';
        document.getElementById('telefono_empresa').textContent =
          userData.telefono || '';
        document.getElementById('correo_empresa').textContent =
          userData.correo || '';
        const direccion = userData.direccion ? userData.direccion.trim() : '';
      ...
      // Mostrar datos para SUPER_ADMIN y AdminSENA
        ...
        const nombreCompleto = `${userData.primer_nombre || ''} ${
          userData.segundo_nombre || ''
        }`.trim();
        const apellidoCompleto = `${userData.primer_apellido || ''} ${
          userData.segundo_apellido || ''
        }`.trim();
        document.getElementById('nombre_admin').textContent = nombreCompleto;
        document.getElementById('apellido_admin').textContent =
          apellidoCompleto;
        document.getElementById('correo_admin').textContent =
          userData.correo || '';
        document.getElementById('telefono_admin').textContent =
          userData.telefono || '';
      } else {
      ...
      function definirTiempoInactividad(rol) {
        if (rol === 'empresa') {
          ...
          mensajeInactividad =
            'Tu sesión de empresa ha finalizado por inactividad.';
        } else if (rol === 'super_admin' || rol === 'AdminSENA') {
          tiempoMaxInactividad = 10 * 60 * 1000; // 10 minutos
          mensajeInactividad =
            'Tu sesión de administrador ha finalizado por inactividad.';
        }
      }
