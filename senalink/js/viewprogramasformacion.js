document.addEventListener('DOMContentLoaded', function () {
  ...
  let estadoActual =
    estadoEnUrl === 'Finalizado' ? 'Finalizado' : 'En ejecucion';

  function obtenerUrlListado(estado) {
    const base = contexto === 'vista1' ? '../../' : '../../../';
    return (
      base +
      'controllers/ProgramaController.php?action=' +
      (estado === 'En ejecucion'
        ? 'listarProgramasEnEjecucion'
        : 'listarProgramasFinalizados')
    );
  }
  ...
  function renderProgramas(lista) {
    if (!container) return;
    container.innerHTML = '';
    // Si hay búsqueda, mostrar todos los resultados sin paginado
    const q = inputBusqueda?.value.trim().toLowerCase();
    let paginaProgramas;
    if (q) {
      paginaProgramas = lista; // Mostrar todos los resultados filtrados
    } else {
      const inicio = (paginaActual - 1) * programasPorPagina;
      const fin = inicio + programasPorPagina;
      paginaProgramas = lista.slice(inicio, fin);
    }
  ...
  if (filtroEjecucion && filtroFinalizado) {
    filtroEjecucion.addEventListener('click', () =>
  ...
  if (!id) {
    if (typeof showAlert === 'function') {
      showAlert(
        'No se encontró el ID del programa. Por favor, accede desde la lista de programas.',
        'error',
        '.container__crud'
      );
    }
  ...
  fetch(urlDetalle, {
    ...
    .then((data) => {
      if (!data.success || !data.programa) {
        if (typeof showAlert === 'function') {
          showAlert(
            'No se encontraron datos para el programa solicitado.',
            'error',
            '.container__crud'
          );
        }
    ...
    .catch((error) => {
      ...
      if (typeof showAlert === 'function') {
        showAlert(
          'Error al cargar los detalles del programa. Por favor, intenta nuevamente.',
          'error',
          '.container__crud'
        );
      }
