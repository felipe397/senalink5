...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function cargarFuncionariosPorEstado(estado) {
        estadoActual = estado;
        fetch(`../../../controllers/UsuarioController.php?action=listarAdminSENA&estado=${estado}&_=${Date.now()}`) // cache busting
    ...
    inputBusqueda.addEventListener("input", function () {
        ...
        const filtrados = q.length >= 1
            ? funcionarios.filter(u => (u.primer_nombre + ' ' + (u.segundo_nombre || '') + ' ' + u.primer_apellido + ' ' + u.segundo_apellido).toLowerCase().includes(q) || u.numero_documento.toLowerCase().includes(q))
            : funcionarios;
